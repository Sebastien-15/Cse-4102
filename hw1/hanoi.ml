...
let rec hanoi_general n from_peg to_peg other_peg : (int * int) list  =
  if n = 0 then []  
    ...
    let first_part = hanoi_general (n-1) from_peg other_peg to_peg in
    let middle_move = [(from_peg, to_peg)] in
    let last_part = hanoi_general (n-1) other_peg to_peg from_peg in
    first_part @ middle_move @ last_part
